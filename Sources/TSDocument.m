...
- (void)windowControllerDidLoadNib:(NSWindowController *)aController
{
	...
	if ((![self isTexExtension:fileExtension]) && ([[NSFileManager defaultManager] fileExistsAtPath:theFileName])) {
...
// edit window appears when opening graphic files and in external editor mode
    ...
    [[NSNotificationCenter defaultCenter] addObserver:pdfView selector:@selector(wasScrolled:) name:NSViewBoundsDidChangeNotification object:[pdfView superview]];
    // end mitsu 1.29

    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(colorizeAll) name:@"NeedsForRecolorNotification" object:nil]; // added by Terada
    ...
    if (!fileIsTex) {
        ...
        if (imageFound) {
            if (self.documentType == isPDF) {
                ...
                if ((self.documentType == isPDF) &&
                    ([SUD boolForKey:PdfFileRefreshKey] == YES) &&
                    ([SUD boolForKey:PdfRefreshKey] == YES)) {
                    self.pdfRefreshTimer = [NSTimer scheduledTimerWithTimeInterval:[SUD floatForKey:RefreshTimeKey]
                                                                            target:self
                                                                          selector:@selector(refreshPDFWindow:)
                                                                          userInfo:nil
                                                                           repeats:YES];
...
// FIX RULER SCROLL
- (void)redrawLineNumbers:(id)sender // added by Terada (for Lion bug)
{
...
- (BOOL)isTexExtension:(NSString *)extension
{
	...
	if (([extension isEqualToString:@"tex"])
		...
		|| ([extension isEqualToString:@"dtx"])
        || ([extension isEqualToString:@"ins"])
		|| ([extension isEqualToString:@"sty"])
        || ([extension isEqualToString:@"cls"])
		|| ([extension isEqualToString:@"Rnw"])
        || ([extension isEqualToString:@"rnw"])
		// added by mitsu --(N) support for .def, .fd, .ltx. .clo
		|| ([extension isEqualToString:@"def"])
        || ([extension isEqualToString:@"fd"])
		|| ([extension isEqualToString:@"ltx"])
        || ([extension isEqualToString:@"clo"])
		// end addition
...
{
	if (([extension isEqualToString:@"dvi"]) || ([extension isEqualToString:@"ps"])
		...
		while ((myRange.location < length) && (!done) && (linesTested < 20)) {
            ...
            if (encodingRange.location != NSNotFound) {
                ...
                if (encodingRange.location != NSNotFound) {
                    ...
                    newEncodingRange.length = [testString length] - newEncodingRange.location;
                    if (newEncodingRange.length > 0) {
                        encodingString = [[testString substringWithRange:newEncodingRange] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                        _encoding = _tempencoding = [[TSEncodingSupport sharedInstance] stringEncodingForKey:encodingString];
                    }
                }
            }
	...
	while ((myRange.location < length) && (!done) && (linesTested < 20)) {
		...
		if (spellcheckRange.location != NSNotFound) {
			...
			if (spellcheckRange.length > 0) {
				spellcheckString = [[testString substringWithRange:spellcheckRange]	stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
- (void)watchFile:(NSString *)fileName
{
    ...
    dispatch_source = dispatch_source_create(DISPATCH_SOURCE_TYPE_VNODE,
                                             fildes,
...
//END PATCH
    ...
    if (content) {
        ...
        if ([SUD boolForKey:ConvertLFKey]) {
            content = [OGRegularExpression replaceNewlineCharactersInString:content withCharacter:OgreLfNewlineCharacter];
        }
        ...
        if ([SUD boolForKey:ptexUtfOutputEnabledKey]) {
            ...
            content = [utfRegex replaceAllMatchesInString:content
                                                 delegate:self
                                          replaceSelector:@selector(decodeUtfStyFormat:contextInfo:)
                                              contextInfo:nil];
        }
...
- (NSDictionary *)fileAttributesToWriteToURL:(NSURL *)absoluteURL ofType:(NSString *)typeName forSaveOperation:(NSSaveOperationType)saveOperation originalContentsURL:(NSURL *)absoluteOriginalContentsURL error:(NSError **)outError
{
	...
	myDictionary = [super fileAttributesToWriteToURL:absoluteURL
                                              ofType:typeName
...
- (void)saveDocument:(id)sender
{
	...
	if (!fileIsTex &&
        [[[self fileURL] path] isEqualToString:[CommandCompletionPath stringByStandardizingPath]]) {
    ...
    if (showFullPath) {
        [textWindow performSelector:@selector(refreshTitle) withObject:nil afterDelay:0.2]; // added by Terada
    }
}
...
- (void)splitPreviewWindow:(id)sender
{
	[self.pdfKitWindow splitPdfKitWindow:(id)sender];
}
...
"*/
{
 ...
 //                                                name:NSApplicationWillTerminateNotification object:nil];
    ...
    if (!skipTextWindow) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(doCompletion:) name:@"completionpanel" object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(doMatrix:) name:@"matrixpanel" object:nil]; // Matrix addition by Jonas

        // added by mitsu --(D) reset tags when the encoding is switched
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetTagsMenu:) name:@"ResetTagsMenuNotification" object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetTagsMenu:) name:@"NSUndoManagerDidRedoChangeNotification" object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetTagsMenu:) name:@"NSUndoManagerDidUndoChangeNotification" object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewBoundsDidChange:) name:NSViewBoundsDidChangeNotification object:[scrollView2 contentView]];
        
        // Register for resizing
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewFrameDidChange:) name:NSViewFrameDidChangeNotification object:textView1];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewFrameDidChange:) name:NSViewFrameDidChangeNotification object:textView2];
    }
	...
	for (i = 0; i < [fileList count]; i++) {
		...
		if ([fm fileExistsAtPath:path isDirectory:&isDirectory]) {
			if (isDirectory) {
				...
				[self makeMenuFromDirectory:submenu basePath:path action:@selector(doTemplate:) level:lv];
	...
	for (i = 0; i < [fileList count]; i++) {
        ...
        if ([fm fileExistsAtPath:path isDirectory:&isDirectory]) {
			if (isDirectory) {
				...
				if (level > 0) {
					submenu = [[NSMenu alloc] init] ;
					[self makeMenuFromDirectory:submenu basePath:path action:action level:level];
					[newItem setSubmenu:submenu];
				}
			} else if ([[[title pathExtension] lowercaseString] isEqualToString:@"tex"]) {
				title = [title stringByDeletingPathExtension];
...
- (void)quitMagnificationPanel:(id)sender
{
	[NSApp endSheet:magnificationPanel returnCode:0];
}

- (void)quitPagenumberPanel:(id)sender
{
...
- (void)chooseProgramEE:(id)sender
{
...
- (void)changeShowFullPath:(id)sender
{
...
- (void)paperSizeOKPressed:(id)sender;
{   
...
- (void)paperSizeCancelPressed:(id)sender;
{
    [PaperSizePanel close];
}
...
- (void)removeNewLinesFromSelection:(id)sender
{
	...
	for (charRange.location = (charRange.location + charRange.length - 1), charRange.length = 1;
         charRange.location > charRangeStart;
         charRange.location--) {
        if ([[textStorageString substringWithRange:charRange] isEqualToString:@"\n"]) {
            [newlineIndexes addObject:[NSNumber numberWithUnsignedInteger:charRange.location]];
        }
	}
...
- (BOOL)fillLogWindow
{
	...
	if ([[NSFileManager defaultManager] fileExistsAtPath:logPath] &&
        [[NSFileManager defaultManager] isReadableFileAtPath:logPath]) {
